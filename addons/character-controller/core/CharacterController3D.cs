...
public partial class CharacterController3D : CharacterBody3D
{
	...
	// Base friction of the character
	...
	{
		Hold,   // Sprint while holding the key
		Toggle  // Toggle sprint on/off with key press
	}
	[Export] public SprintMode SprintModeType = SprintMode.Hold;
	...
	// True if sprint is toggled on
	private bool _isSprintToggled = false;
	// Time of last sprint toggle
	private float _lastSprintToggleTime = 0f;
	...
	private const float MIN_STEPS_PER_SECOND = 0f;
	private const float MAX_STEPS_PER_SECOND = 4.0f;
	...
	public virtual void Move(float delta, Vector2 inputAxis = new Vector2(), bool inputJump = false, bool inputCrouch = false, bool inputSprint = false, bool inputSwimDown = false, bool inputSwimUp = false)
	{
		...
		if (GlobalPosition != _lastPosition)
		{
			_lastMovementTime = Time.GetTicksMsec();
		}
		...
		// Permettre le saut pendant le slide
		bool canJump = IsOnFloor() && !HeadCheck.IsColliding();
		if (inputJump && canJump)
		{
			// Désactiver le slide si on saute
			SlideAbility.SetActive(false);
		}
		JumpAbility.SetActive(inputJump && canJump);
		...
		// Gestion du sprint selon le mode
		bool canSprint = !IsCrouching() && !IsFlyMode() && !SwimAbility.IsFloating() && !SwimAbility.IsSubmerged();
		if (SprintModeType == SprintMode.Hold)
		{
			// Mode Hold : sprint uniquement si on avance
			SprintAbility.SetActive(inputSprint && canSprint && inputAxis.X >= 0.5);
		}
		else
		{
			...
			// Désactiver le sprint si on ne bouge pas depuis trop longtemps
			if (_isSprintToggled && Time.GetTicksMsec() - _lastMovementTime > SPRINT_DISABLE_DELAY * 1000)
			{
				_isSprintToggled = false;
			}

			SprintAbility.SetActive(_isSprintToggled && canSprint);
		}
		
		// Gestion du slide
		bool isSprinting = SprintModeType == SprintMode.Hold ? inputSprint : _isSprintToggled;
		SlideAbility.SetActive(isSprinting && inputCrouch && IsOnFloor() && !IsFloating() && !IsSubmerged() && !IsFlyMode());
	...
	public bool Step(bool isOnFloor)
	{
		...
		if (isOnFloor)
		{
			_lastStepTime = Time.GetTicksMsec() / 1000.0f;
	...
	public bool IsStep(float velocity, bool isOnFloor, float delta)
	{
		...
		float speedRatio = velocity / _currentMaxSpeed;
		float stepsPerSecond = MIN_STEPS_PER_SECOND + (MAX_STEPS_PER_SECOND - MIN_STEPS_PER_SECOND) * speedRatio;
		float stepDelay = stepsPerSecond > 0 ? 1.0f / stepsPerSecond : float.MaxValue;

		if (currentTime - _lastStepTime >= stepDelay)
		{
